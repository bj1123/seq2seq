...
def get_paths(gt_path):
    src_paths = files_including(gt_path, 'test.complex')
    ref_paths = files_including(gt_path, 'test.simple')
...
def read_dfs(paths, dec):
    ...
    for i in paths:
        ...
        res.append(list(map(lambda x: ' '.join(map(str, x[1:-1])), texts)))
    if len(res) ==1:
        return res[0]
    else:
        return list(zip(*res))
...
def read_json(paths):
    temp = load_json(paths)
    return [' '.join(map(str, x)) for x in temp]

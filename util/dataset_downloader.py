...
from util.tokenize import LMIndexer
import json
import argparse
...
def prepair_dataset(root, dataset_name='wiki103', tokenizer_class=tokenizers.SentencePieceBPETokenizer, vocab_size=30000):
    ...
    encoded_savepath = os.path.join(os.path.join(root,acc_to_full[dataset_name]),
                                    'encoded_{}_{}'.format(tokenizer_savenames[tokenizer_class], vocab_size))
